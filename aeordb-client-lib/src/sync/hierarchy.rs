...
pub fn is_excluded_by_child(remote_path: &str, exclusions: &[String]) -> bool {
  exclusions
    .iter()
    .any(|exclusion| remote_path.starts_with(exclusion))
}
...
mod tests {
  ...
  fn test_is_excluded_by_child() {
    ...
    assert!(is_excluded_by_child(
      "/projects/secrets/key.json",
      &exclusions
    ));
    assert!(is_excluded_by_child(
      "/projects/secrets/deep/nested.txt",
      &exclusions
    ));
  ...
  fn test_is_local_excluded_by_child() {
    ...
    assert!(is_local_excluded_by_child(
      std::path::Path::new("/home/u/projects/secrets"),
      &exclusions
    ));
    assert!(is_local_excluded_by_child(
      std::path::Path::new("/home/u/projects/secrets/key.json"),
      &exclusions
    ));
    assert!(!is_local_excluded_by_child(
      std::path::Path::new("/home/u/projects/readme.md"),
      &exclusions
    ));
  }
