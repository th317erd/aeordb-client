...
fn test_list_conflicts_empty_database() {
  ...
  let conflicts = list_conflicts_typed(state.engine()).expect("list_conflicts_typed failed");
...
async fn test_conflicts_http_api_list_empty() {
  ...
  let response = client
    .get(format!("{}/api/v1/conflicts", base_url))
    .send()
    .await
    .expect("list failed");
  ...
  let response = client
    .post(format!("{}/api/v1/conflicts/dismiss-all", base_url))
    .send()
    .await
    .expect("dismiss-all failed");
  assert_eq!(response.status(), 200);
