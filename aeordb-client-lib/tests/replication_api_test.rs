use aeordb::engine::{
  DirectoryOps, RequestContext, StorageEngine, apply_sync_chunks, compute_sync_diff, file_history,
  get_needed_chunks, list_conflicts_typed,
};
...
fn test_compute_sync_diff_on_empty_db() {
  ...
  let diff = compute_sync_diff(&engine, None, None, false).expect("compute_sync_diff failed");
...
fn test_compute_sync_diff_detects_new_file() {
  ...
  // Store a file
  ops
    .store_file_buffered(&ctx, "/docs/hello.txt", b"hello world", Some("text/plain"))
    .expect("store failed");

  // Diff from scratch — should see the file as added
  let diff = compute_sync_diff(&engine, None, None, false).expect("diff failed");
...
fn test_list_conflicts_typed_empty() {
  ...
  let conflicts = list_conflicts_typed(&engine).expect("list_conflicts_typed failed");

  assert!(
    conflicts.is_empty(),
    "new database should have no conflicts"
  );
}
...
fn test_file_history_empty() {
  ...
  let history = file_history(&engine, "/nonexistent.txt").expect("file_history failed");

  assert!(
    history.is_empty(),
    "nonexistent file should have no history"
  );
}
...
fn test_get_needed_chunks_empty() {
  ...
  let chunks = get_needed_chunks(&engine, &[fake_hash]).expect("get_needed_chunks failed");
...
fn test_apply_sync_chunks_empty() {
  ...
  let count = apply_sync_chunks(&engine, &[]).expect("apply_sync_chunks failed");
