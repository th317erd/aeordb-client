...
fn test_reopen_existing_database() {
  ...
  // Create and write something
  {
    ...
    store
      .store_json("/settings/test.json", &serde_json::json!({"key": "value"}))
...
#[tokio::test]
async fn maintenance_timer_flushes_embedded_state_for_dirty_drop() {
  let path = temp_database_path();
...

  {
    let store = StateStore::open_or_create(&path).expect("failed to reopen");
    let value: Option<serde_json::Value> = store
      .read_json("/settings/timer-flushed.json")
      .expect("failed to read");
...
fn test_client_identity_stable_across_calls() {
  ...
  let identity_1 = store
    .get_or_create_identity()
    .expect("failed to get identity");
  let identity_2 = store
    .get_or_create_identity()
    .expect("failed to get identity");
...
fn test_store_and_read_json() {
  ...
  store
    .store_json("/settings/test-data.json", &data)
    .expect("failed to store");

  let read_back: Option<TestData> = store
    .read_json("/settings/test-data.json")
    .expect("failed to read");
...
fn test_read_json_nonexistent_returns_none() {
  ...
  let result: Option<serde_json::Value> = store
    .read_json("/settings/nope.json")
    .expect("failed to read");
...
fn test_exists_and_delete() {
  ...
  store
    .store_json("/settings/deleteme.json", &serde_json::json!({"x": 1}))
    .expect("failed to store");

  assert!(
    store
      .exists("/settings/deleteme.json")
      .expect("exists check failed")
  );

  store
    .delete("/settings/deleteme.json")
...
fn test_list_directory() {
  ...
  store
    .store_json("/settings/a.json", &serde_json::json!({}))
    .expect("failed to store");
  store
    .store_json("/settings/b.json", &serde_json::json!({}))
    .expect("failed to store");
  store
    .store_json("/settings/c.json", &serde_json::json!({}))
    .expect("failed to store");
...
fn test_store_json_overwrites_existing() {
  ...
  store
    .store_json("/settings/data.json", &serde_json::json!({"v": 1}))
    .expect("failed to store v1");

  store
    .store_json("/settings/data.json", &serde_json::json!({"v": 2}))
    .expect("failed to store v2");

  let value: Option<serde_json::Value> = store
    .read_json("/settings/data.json")
    .expect("failed to read");
